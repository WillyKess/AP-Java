...
public class EndOfUnitProject {
    ...
    public static int blackjack() {
        ...
        int int1 = readin.nextInt();
        int int2 = readin.nextInt();
        int result = 1234; // Have to declare this to a value, unless we want to have a whole bunch of ternary operators instead of if statements. At least it gives us an easy
                           // way to tell if the input is wrong.
        if (int1 >= 0 && int2 >= 0) {
            if (int1 > 21 && int2 > 21) {
                result = 0;
            } else if (int1 == int2) {
